...
# stt, name, order ("by-date", "most-upvoted") , cursor
...
curl 'https://www.producthunt.com/frontend/graphql' \
  ...
  "variables":{"slug":"'$2'","order":"'$3'","cursor":"'$4'","query":null,"includeLayout":false},
  "query":"query TopicPage($slug:String\u0021$cursor:String$query:String$subtopic:ID$order:String\u0021)
  {
    topic(slug:$slug){
      id slug name parent{id name slug __typename}
      posts(first:20 after:$cursor query:$query subtopic:$subtopic order:$order)
        {edges
          {node
            {id name slug tagline pricingType commentsCount ...on Votable{id votesCount} updatedAt featuredAt createdAt disabledWhenScheduled ...TopicPageReviewRatingFragment __typename}
          __typename}
  ...
  fragment TopicPageReviewRatingFragment on Post{id product{id reviewsWithBodyCount slug __typename}__typename}"
}
    ' \
  --compressed > "tmp/_r.$1.$2.$3.$4.ongoing"
  mv "tmp/_r.$1.$2.$3.$4.ongoing" "tmp/_r.$1.$2.$3.$4.json"
  
  # 21/9 mat nhung attr nay: reviewsWithBodyCount reviewsCount reviewsRating