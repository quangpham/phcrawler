  ...
  "query":"query PostPageSocialProof($postId:ID\u0021$limit:Int\u0021)
    {post(id:$postId)
      {id contributors(limit:$limit)
        {role user
          {id ...UserImage __typename}
        __typename}
      __typename}
    }
    fragment UserImage on User{id name username avatarUrl headline twitterUsername websiteUrl isViewer  followersCount followingsCount isViewer isMaker isTrashed badgesCount badgesUniqueCount ...KarmaBadgeFragment createdAt __typename}
    fragment KarmaBadgeFragment on User{id karmaBadge{kind score __typename}__typename}"
  }
    ' \
...
mv "tmp/_r.$1.ongoing" "tmp/_r.$1.json"
  ...
  # fragment UserItemFragment on User{id name username headline twitterUsername websiteUrl url avatarUrl followersCount followingsCount isViewer isMaker isTrashed ...KarmaBadgeFragment createdAt __typename}